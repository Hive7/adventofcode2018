...
for line in lines:
    ...
    if line[-1:] == "p":
        ...
        if guard not in days:
            days[guard] = {"wake": list(), "sleep": list(), "total": 0, "times": {}, "max": 0, "maxTime": 0}
        if line[-2:] == "up":
            ...
            if dateS == dateW and timeW < timeS:
                for i in range(timeW, timeS):
                    try:
                        ...
                        days[guard]["times"][i] = 1
                    if days[guard]["times"][i] > days[guard]["max"]:
                        days[guard]["max"] = days[guard]["times"][i]
                        days[guard]["maxTime"] = i
                    days[guard]["total"] += 1
                    if days[guard]["total"] > max:
                        max = days[guard]["total"]
                        maxGuard = guard
                break

print(maxGuard * days[maxGuard]["maxTime"])
